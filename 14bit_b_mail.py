...
#画腔
dy=0#-250000
painter3=paintlib.CavityPainter(pya.DPoint(0,1050000+dy),angle=-90,widout=20000,widin=10000,bgn_ext=0,end_ext=0)
...
def path1(painter):#设置内轮廓路径
    dl=0#547.0693762227893-dy/11+0/22
    length=0
    length+=painter.Straight(85000+dy)
    length+=painter.Turning(55000)
    length+=painter.Straight(980000+dl)
    length+=painter.Turning(-55000)
    length+=painter.Turning(-55000)
    for i in range(4):
        length+=painter.Straight(2134000+dl*2)#1
        length+=painter.Turning(55000)
        length+=painter.Turning(55000)
        length+=painter.Straight(2134000+dl*2)#2
        length+=painter.Turning(-55000)
        length+=painter.Turning(-55000)
    length+=painter.Straight(2134000+dl*2)#1
    length+=painter.Turning(55000)
    ...
    length+=painter.Straight(1794000+dl)
    length+=painter.Turning(-55000)
    length+=painter.Turning(-55000)
    
    length+=painter.Straight(1416000+dl*2)#1
    length+=painter.Turning(55000)
    length+=painter.Turning(55000)
    
    length+=painter.Straight(782000+dl)
    length+=painter.Turning(-55000)
...
def airbrigeDistance(distance,first=500000,second=480000,staticList=[0,0,0]):
...
class TransfilePainter2(Painter):
            ...
            if (icell.name == self.insertcellname):
                ...
                for cpts in centerlinelist:
                    ...
                    for i,pt in enumerate(cpts[1:-1],1):
                        ...
                        if airbrigeDistance(distance) !=dt_int:
                            ...
                            dt_int=dt_int+1
                    print('draw airbrige num :',dt_int)
        for icell in IO.layout.top_cells():
            if (icell.name == self.insertcellname):
                icell.flatten(True)
                icell.delete()
...
painter4=TransfilePainter2("[Crossover48].gds","insert")
