...
IO=paintlib.IO
# matlabtpl
with open(IO.path+'/matlabtplabt2n1novta.m') as fid:
    _matlabtpl=fid.read()
...
class Simulation:
    ...
    @staticmethod
    def _get_region_cell_port(region,brush,layerlist,boxx,boxy,offsetx=0,offsety=0,deltaangle=0,absx=None,absy=None,portbrushs=None,transmissionlines=None):#added crossoverLayerList
    ...
    @staticmethod
    def _format_region_into_matlab_code(region,regionvia,regionbr,name,prefix=''):
        output=[]
        def pushln(ss):
    ...
    @staticmethod
    def create(name,startfrequency,endfrequency,stepfrequency,layerlist,layerlistvia,layerlistbr,boxx,boxy,region,brush,transmissionlines=None,portbrushs=None,offsetx=0,offsety=0,deltaangle=0,absx=None,absy=None):#added crossoverLayerList
        '''
        frequency单位GHz
        '''
        ...
        final_regionvia1,_,_=Simulation._get_region_cell_port(
            region=region,brush=brush,layerlist=layerlistvia,boxx=boxx,boxy=boxy,deltaangle=deltaangle,absx=absx,absy=absy,portbrushs=portbrushs,transmissionlines=transmissionlines
        )
		
        final_regionbr1,_,_=Simulation._get_region_cell_port(
            region=region,brush=brush,layerlist=layerlistbr,boxx=boxx,boxy=boxy,deltaangle=deltaangle,absx=absx,absy=absy,portbrushs=portbrushs,transmissionlines=transmissionlines
        )
        ...
        def pushln(ss):
            output.append(prefix+ss+'\n')
        output.extend(Simulation._format_region_into_matlab_code(region=final_region,regionvia=final_regionvia1, regionbr=final_regionbr1,name=name,prefix=prefix))
