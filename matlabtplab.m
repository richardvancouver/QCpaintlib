...
for ii = numindex:size(clines,2)
    if size(clines{ii},2)>=7 && strcmp(clines{ii}(1:11) , 'VIA POLYGON')%find the end  endindex=ii
        break
    end
...
for ii = boxindex:size(clines,2)
    if size(clines{ii},2)>=3 && strcmp(clines{ii}(1:3) , 'TOLEVEL') %%begining of the insertindex
        beginindexindex=ii;
        break
    end
end
...
for ii = numindex:size(clines,2)
    if size(clines{ii},2)>=7 && strcmp(clines{ii}(1:3) , 'END') && ~strcmp(clines{ii+1}(1:3) , 'VIA') %%%end of the insertindex find the 'end' with no 'via' following
        endindex=ii;
		break
    end
    if size(clines{ii},2)>=3 && strcmp(clines{ii}(1:7) , 'TOLEVEL')
        insertindexs(end+1)=ii;
    end
end
insertindexs=insertindexs(1:end-1);
...
lastindex=0;
for ii = insertindexs
    templines=cat(2,templines,clines(lastindex+1:ii),{'TLAYNAM Stream2:2 INH'}); %keep appending stuff, mainly the end-to-end sections plus 'TLAY...' to templines.
    lastindex=ii+1;
end
clines=cat(2,templines,clines(lastindex+1:end));
...
for ii = boxindex:size(clines,2)
    if size(clines{ii},2)>=3 && strcmp(clines{ii}(1:3) , 'TOLEVEL') %%
        numindex=ii;
        break
    end
end
...
insertindexs(end+1)=numindex;
for ii = numindex:size(clines,2)
    if size(clines{ii},2)>=7 && strcmp(clines{ii}(1:3) , 'END') && ~strcmp(clines{ii+1}(1:3) , 'VIA') %%%begining of the insertindex
        beginindex=ii;
		break
    end
...
lastindex=0;
for ii = insertindexs
    templines=cat(2,templines,clines(lastindex+1:ii+1),{'TLAYNAM Stream3:0 INH'}); %keep appending stuff, mainly the end-to-end sections plus 'TLAY...' to templines.
    lastindex=ii+1;
end
clines=cat(2,templines,clines(lastindex+1:end));
