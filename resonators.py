...
import math
#wf2=[6.75 7]
wf=6.425*2*math.pi #in GHz
epsilonr=11.4
epsiloneff=(1+epsilonr)/2
print("pi: %s" %(math.pi))
print("sqrt: %s" %(math.sqrt(1.44)))
cavitylen=math.pi*3e8/(2*wf*math.sqrt(epsiloneff)) #in nm
...
def path2(painter):#设置内轮廓路径
    ...
    lenthunit=0
    lenthunit+=painter.Straight(104500)#1
    lenthunit+=painter.Turning(-30000)
    lenthunit+=painter.Turning(-30000)
    lenthunit+=painter.Straight(104500)#2
    lenthunit+=painter.Turning(30000)
    lenthunit+=painter.Turning(30000)


    frac=cavitylen/lenthunit
    print("sections: %s"%(frac))
    print("sections: %s"%( math.floor(frac) ))
    remainder=cavitylen-math.floor(frac)*lenthunit
    print("remainder: %s"%( remainder ))

    while length<(cavitylen-lenthunit):
        #oldlength=length
        length+=painter.Straight(104500)#1
        length+=painter.Turning(-30000)
        length+=painter.Turning(-30000)
...
#lengthunit=painter3.Run(pathunit)
#print("lenthunit: %s"%( lengthunit ))

def pathentry(painter):
    lengthentry=0
    lengthentry+=painter.Straight(100000)
    lengthentry+=painter.Turning(-30000)
    lengthentry+=painter.Straight(0)
    lengthentry+=painter.Turning(30000)
    lengthentry+=painter.Turning(30000)
    return lengthentry
...
def path3(painter):#设置内轮廓路径
    ...
    length+=painter.Straight(100000)
    length+=painter.Turning(-30000)
    length+=painter.Straight(remainder)
    length+=painter.Turning(30000)
    length+=painter.Turning(30000)
