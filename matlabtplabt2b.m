...
for ii = numindex:size(clines,2)
    if size(clines{ii},2)>=7 && strcmp(clines{ii}(1:7) , 'VIA POL')%find the end  endindex=ii
        break
    end
...
for ii = boxindex:size(clines,2)
    if size(clines{ii},2)>=3 && strcmp(clines{ii}(1:7) , 'TOLEVEL') %%begining of the insertindex
        beginindexindex=ii;
        break
    end
end
