...
class Eraser(object):
    ...
    #@staticmethod
    def eraserpoly(self, rectangle3,rectangledielectric):
          ...
          for jj in range(rectangledielectric.num_points_hull()):
              if  rectangle3.inside(rectangledielectric.point_hull(jj)):
                self.list2.append( rectangledielectric.point_hull(jj) )
          ...
          for edg1 in rectangledielectric.each_edge():
              for edg2 in rectangle3.each_edge():
                 if edg1.intersect(edg2):
                     ...
                     self.listin.append( edg1.intersection_point(edg2) )
...
#          print(self.listin[1])
          ...
          self.listinhole=[]
          for it in self.listin:
              self.listinhole.append( pya.DPoint(it.x, it.y) )
          
          for bt in self.list2:
              self.listinhole.append( pya.DPoint(bt.x,bt.y) )
          
          #rectangle3.insert_hole( [pya.DPoint(10000,10000),pya.DPoint(10000,30000),pya.DPoint(90000,30000),pya.DPoint(90000,10000)] )
          
          rectangle3.insert_hole( self.listinhole ) #need topological sorting before inputting listinhole
          return self.listinhole[0]#rectangle3#listinhole
